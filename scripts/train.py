...
def main():
    ...
    print("[INFO] Initializing KITTI Train and Val Datasets...", flush=True)
    ...
    print("[INFO] Initializing Mono3D Network & MultiTask Loss...", flush=True)
    ...
    print(f"[INFO] Starting Training Loop ({epochs} Epochs)...", flush=True)

    for epoch in range(1, epochs + 1):
        model.train()
        train_loss = 0.0
        
        # Real-time tqdm Progress Bar for Training
        ...
        print(f"--> [EPOCH {epoch}/{epochs} SUMMARY] Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}", flush=True)
